...
const DEFAULT_USER_ID = '14b9573d-f555-4e8c-83d1-16dfcbf29b4a';
 ...
 */
async function handleHttpRequest(request, config) {
	...
	if (pathname === '/scamalytics-lookup') {
		...
		return await fetchScamalyticsData(ip, config.scamalytics);
	}
	...
	if (matchedUUID) {
		...
		if (pathname === `/ipsub/${matchedUUID}`)
			 return await generateIpSubscription(matchedUUID, host);
	}
    ...
    <body>
        <div class="container">
            ...
            <p>Please provide your User ID (UUID) in the URL to access the endpoints. Replace <code>${
							config.userID.split(',')[0]
						}</code> with your own UUID.</p>
		...
		.pipeTo(
			new WritableStream({
				async write(chunk) {
					...
					const { addressRemote, portRemote, addressType, rawDataIndex } =
						parsed;
...
function createRequestConfig(request, env) {
	...
	if (qs.get('socks5_relay'))
		cfg.socks5Relay = qs.get('socks5_relay') === 'true';
	...
	if (cfg.enableSocks)
		cfg.parsedSocks5Address = socks5AddressParser(cfg.socks5Address);
	return cfg;
}
...
function getBeautifulConfig(userID, hostName, proxyIPWithPort) {
	...
	const dreamConfig = `vless://${userID}@${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=chrome&type=ws&host=${hostName}&path=${encodeURIComponent(
		vlessPath
	)}#${hostName}-Xray`;
	const freedomConfig = `vless://${userID}@${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=firefox&type=ws&host=${hostName}&path=${encodeURIComponent(
		vlessPath
	)}#${hostName}-Singbox`;
...
function GenSub(userID_path, hostname, proxyIPArray) {
	...
	const commonUrlPartHttp = `?encryption=none&security=none&fp=firefox&type=ws&host=${hostname}&path=${encodeURIComponent(
		randomPath()
	)}#`;
	const commonUrlPartHttps = `?encryption=none&security=tls&sni=${hostname}&fp=chrome&type=ws&host=${hostname}&path=${encodeURIComponent(
		randomPath()
	)}#`;
...
function GenIpSub(userID, hostname, ips) {
	const configs = [];
	const vlessPath = `/?${CONSTANTS.URL_ED_PARAM}`;
	const commonUrlPart = `?encryption=none&security=tls&sni=${hostname}&fp=chrome&type=ws&host=${hostname}&path=${encodeURIComponent(
		vlessPath
	)}#`;

	ips.forEach(ip => {
		const configName = `REvil-${ip}`;
		const vlessUrl = `${CONSTANTS.VLESS_PROTOCOL}://${userID}${
			CONSTANTS.AT_SYMBOL
		}${ip}:443${commonUrlPart}${encodeURIComponent(configName)}`;
...
async function generateIpSubscription(matchingUserID, host) {
	try {
		...
		const data = await response.json();
		const ips = [...(data.ipv4 || []), ...(data.ipv6 || [])].map(
			item => item.ip
		);
		...
		return new Response(
			`Failed to generate IP subscription: ${error.message}`,
			{ status: 500 }
		);
	}
}
 ...
 * @param {ArrayBuffer} buffer - The incoming data chunk.
 * @param {string} configuredIDs - The configured UUID(s) for the worker.
...
function ProcessProtocolHeader(buffer, configuredIDs) {
	...
	const recvUUID = stringify(
		new Uint8Array(buffer.slice(off.UUID, off.UUID + 16))
	);
	...
	switch (aType) {
		case 1:
			aLen = 4;
			addr = Array.from(
				new Uint8Array(buffer.slice(addrIdx, addrIdx + 4))
			).join('.');
			break;
		case 2:
					...
					if (webSocket.readyState === CONSTANTS.WS_READY_STATE_OPEN) {
						...
						if (protocolResponseHeader) {
							const newChunk = new Uint8Array(
								protocolResponseHeader.length + chunk.length
							);
			...
			webSocketServer.addEventListener('message', event => {
				try {
					...
					if (typeof data === 'string') {
						...
						data
							.arrayBuffer()
							.then(buf => controller.enqueue(buf))
